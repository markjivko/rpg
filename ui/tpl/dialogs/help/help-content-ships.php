...
/* @var $configObject Stephino_Rpg_Config_Ship */
...
foreach (Stephino_Rpg_Config::get()->ships()->getAll() as $shipConfig) {
    $shipConfig->getArmour() > $maxArmour && $maxArmour = $shipConfig->getArmour();
    $shipConfig->getAgility() > $maxAgility && $maxAgility = $shipConfig->getAgility();
    $shipConfig->getDamage() > $maxDamage && $maxDamage = $shipConfig->getDamage();
    $shipConfig->getAmmo() > $maxAmmo && $maxAmmo = $shipConfig->getAmmo();
    $shipConfig->getLootBox() > $maxLootBox && $maxLootBox = $shipConfig->getLootBox();
}
?>
<div class="row p-2 text-center justify-content-center">
    <div class="col-12">
        <h5>
            <?php echo Stephino_Rpg_Config::get()->core()->getConfigShipsName(true);?> &#8250; <b><?php echo $configObject->getName(true);?></b>
        </h5>
    </div>
    <?php 
        if (
            null !== $configObject->getBuilding()
            && (count($costData) || count($productionData))
        ):
    ?>
        <div class="col-10">
            <div class="label ship-level-number">
                <span>
                    <span
        ...
        <div class="col-10">
            <div class="label ship-count-number">
                <span>
                    ...
                    <input 
                        ...
                        title="<?php echo esc_attr__('Use this to preview fleet costs, abilities etc.', 'stephino-rpg');?>" 
                        class="form-control" />
                </span>
            </div>
        </div>
...
?>
<?php if ($configObject->getCivilian() && ($configObject->getAbilityColonize() || $configObject->getAbilityTransport())):?>
    <div class="col-12 p-2">
        ...
        <ul>
            <?php if ($configObject->getAbilityColonize()):?>
                <li>
                    <b><?php echo esc_html__('Colonizer', 'stephino-rpg');?>:</b> <?php
                        echo esc_html__('claim empty lots, expanding your empire', 'stephino-rpg');
                    ?>
                </li>
            <?php endif;?>
            <?php if ($configObject->getAbilityTransport()):?>
                <li>
                    <b><?php echo esc_html__('Transporter', 'stephino-rpg');?></b>: <?php
                        echo sprintf(
                            'carry up to %s (%s, %s and resources) between %s',
                            '<b>' . $configObject->getAbilityTransportCapacity() . '</b>',
...
<?php if (!$configObject->getCivilian()):?>
    <div class="col-12 p-2">
        ...
        <ul>
            ...
            <?php if (Stephino_Rpg_Config::get()->core()->getTravelTime() > 0 && $configObject->getTroopCapacity()): ?>
                <li><b><?php echo esc_html__('Troops capacity', 'stephino-rpg');?></b>: <b><?php echo number_format($configObject->getTroopCapacity());?> </b></li>
            <?php endif;?>
        </ul>
        <div class="row">
            <?php
...
<div class="col-12 p-2">
    ...
    <ul>
        <?php if (null !== $configObject->getBuilding()):?>
            ...
            <?php if (!$configObject->getCivilian() || !$configObject->getAbilityTransport()):?>
                <li>
                    <b><?php echo esc_html__('Mass', 'stephino-rpg');?></b>: <b><?php echo $configObject->getTransportMass();?> </b>
                    <?php 
                        echo sprintf(
